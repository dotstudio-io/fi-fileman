...
function save(tempfile, destpath, done) {
  debug("Moving temp file " + tempfile.path);
...
function multiparser(req, res, next) {
  ...
  busboy.on('file', function (fieldname, file, filename, encoding, mimetype) {
    ...
    var filepath = path.normalize(path.join(TEMP_DIR, outfile));
    var ws = fs.createOutputStream(filepath);

    file.on('end', function () {
      debug("Multiparser: wrote temp file: " + filepath);
    ...
    file.on('error', function (err) {
      debug("Mutiparser: file stream write error!");
      next(err);
    });
  ...
  fs.exists(resolved, function (exists) {
    ...
    fs.stat(resolved, function (err, stats) {
      ...
      magic.detectFile(resolved, function (err, mimetype) {
        if (err) {
          debug("Downloader: get file mimetype error!");
          return next(err);
        }
        ...
        rs.on('error', function (err) {
          debug("Downloader: file read stream error!");
          next(err);
        });
...
function cleaner(req, res, next) {
  res.on('finish', function () {
    ...
    if (type.is(req.files, Array) && req.files.length) {
      debug("Cleaning " + req.files.length + " uploaded files...");

      /* Unlink each file */
      req.files.forEach(function (file) {
        fs.remove(file.path, function (err) {
          if (err) {
            debug("Couldn't clean uploaded file");
            debug(file);
            debug(err);
          }
        });
